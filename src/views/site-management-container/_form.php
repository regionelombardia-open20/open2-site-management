...
<div class="site-management-container-form col-xs-12 nop">
        ...
        <?php if(!$model->isNewRecord){ ?>
            ...
            <div class="col-lg-6 col-sm-12">
                <?= $form->field($model, 'fixed_template_id')->widget(Select2::className(),[
                        'data' => \yii\helpers\ArrayHelper::map(\amos\sitemanagement\models\base\SiteManagementTemplate::find()->all(), 'id', 'name'),
                        'options' => ['placeholder' => Module::t('amossistemanagement', 'Select...')]
                ]) ?>
            <?php  } ?>
        </div>
    ...
    <div class="col-lg-12 col-sm-12">
        ...
        if (!$model->isNewRecord) { ?>
            <?= Html::a(\Yii::t('amossitemanagement', '#add_elements'), ['/sitemanagement/site-management-container-element-mm/create', 'idContainer' => $model->id], ['class' => 'btn btn-navigation-primary']) ?>
                        ...
                        'value' => function ($model) use ($max, $min) {
                            ...
                            if($model->elem_order != $min) {
                                $buttons .= Html::a(\open20\amos\core\icons\AmosIcons::show('long-arrow-up'), [
                                    '/sitemanagement/site-management-container-element-mm/order-container', 'id' => $model->id, 'container_id' => $model->container_id, 'direction' => 'up'],
                                    ['class' => 'btn btn-tools-secondary']);
                            }
                            if($model->elem_order != $max) {
                                $buttons .= Html::a(\open20\amos\core\icons\AmosIcons::show('long-arrow-down'), [
                                    '/sitemanagement/site-management-container-element-mm/order-container', 'id' => $model->id, 'container_id' => $model->container_id, 'direction' => 'down'],
                                    ['class' => 'btn btn-tools-secondary']);
                            }
                            return $buttons;
                        },
                        ...
                        'template' => '{updateElement}{update}{delete}',
                            'buttons' => [
                                    'updateElement' => function($url, $model){
                                        return Html::a(\open20\amos\core\icons\AmosIcons::show('edit'), ['/sitemanagement/site-management-element/update', 'id'=> $model->element_id],[
                                            'class' => 'btn btn-tools-secondary',
